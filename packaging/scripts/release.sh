...
build_release() {
    local SUFFIX="$1"
    local OUTDIR="$BASE/packaging/dist/ddt_client_$SUFFIX"
    ...
    # CMake 模块
    if [ -d "$BASE/src/client/cmake" ]; then
        cp "$BASE/src/client/cmake/"*.cmake "$OUTDIR/src/cmake/" 2>/dev/null || true
    fi
    ...
    # 共享代码层 (common)
    if [ -d "$BASE/src/common" ]; then
        mkdir -p "$OUTDIR/src/common"
        cp "$BASE/src/common/"*.h "$OUTDIR/src/common/"
        cp "$BASE/src/common/"*.cc "$OUTDIR/src/common/"
    fi
    ...
    cp "$BASE/src/client/assets/fonts/"* "$OUTDIR/assets/fonts/" 2>/dev/null || true
    # 复制所有游戏贴图 PNG
    for png in "$BASE/src/client/assets/"*.png; do
        [ -f "$png" ] && cp "$png" "$OUTDIR/assets/"
    done
    echo "  Assets: $(ls "$OUTDIR/assets/"*.png 2>/dev/null | wc -l) PNG files"
    if [ -d "$BASE/src/client/shaders" ]; then
        cp -r "$BASE/src/client/shaders" "$OUTDIR/shaders"
    fi

    # Proto source
    cp "$BASE/src/proto/ddt.proto" "$OUTDIR/proto/"

    # CMakeLists（standalone 版本）
    if [ -f "$BASE/src/client/CMakeLists.txt.standalone" ]; then
        cp "$BASE/src/client/CMakeLists.txt.standalone" "$OUTDIR/CMakeLists.txt"
    fi
