...
LobbyRoom* LobbyServiceImpl::findRoomByAccountLocked(uint64_t accountId) {
    for (auto& kv : m_rooms) {
        for (int i = 0; i < kv.second->seatCount; ++i) {
            if (kv.second->seats[i].accountId == accountId) {
...
void LobbyServiceImpl::broadcastRoomUpdate(uint32_t roomId) {
    ...
    // 包进 RoomUpdateNotify(客户端用 RoomUpdateNotify.Parser 解析)
...
void LobbyServiceImpl::broadcastRoomListToAll() {
    ...
    // 用注入的 pushAll 闭包推给所有在线玩家
    if (m_pushAll) {
        m_pushAll(MSG_ROOM_LIST_NOTIFY, payload);
    }
}
...
void LobbyServiceImpl::JoinRoom(::google::protobuf::RpcController*,
    ...
    if (team == TEAM_RED && redCnt >= kPerTeam) {
        team = TEAM_BLUE;
    } else if (team == TEAM_BLUE && blueCnt >= kPerTeam) {
        team = TEAM_RED;
    }
...
void LobbyServiceImpl::LeaveRoom(::google::protobuf::RpcController*,
    ...
    uint32_t rid = 0;
    {
        ...
        if (idx >= 0) {
            for (int i = idx; i < room->seatCount - 1; ++i) {
                room->seats[i] = room->seats[i + 1];
            }
            room->seatCount--;
        }
        // 战斗结束后 LeaveRoom: 重置 started + 清 ready, 让房间重新可用
        if (room->started) {
            room->started = false;
            for (int i = 0; i < room->seatCount; ++i) {
...
void LobbyServiceImpl::Ready(::google::protobuf::RpcController*,
    ...
    uint32_t rid = 0;
    {
        ...
        rid = room->roomId;
        // 战斗结束回归: 房间仍处于 started 时再次准备表示"开始新一局",
        // 重置 started + 清所有人 ready 后进入新一轮准备流程
        if (room->started) {
            room->started = false;
            for (int i = 0; i < room->seatCount; ++i) {
                room->seats[i].ready = false;
...
// 尝试开局: 锁内拷贝玩家快照 + 置 started, 锁外 RPC 调 EnterBattle
void LobbyServiceImpl::tryStart(uint32_t roomId) {
    ...
    bool shouldStart = false;
    {
        ...
        if (room->seatCount < 2 || room->started) {
            return;
        }
        ...
        if (!allReady || redReady < 1 || blueReady < 1) {
            return;
        }
...
void LobbyServiceImpl::Chat(::google::protobuf::RpcController*,
    ...
    if (m_push) {
        ...
        if (room) {
            ...
            for (int i = 0; i < room->seatCount; ++i) {
                if (room->seats[i].accountId == req->account_id()) {
                    senderTeam = room->seats[i].team;
                    break;
                }
            }
            std::vector<RoutingHandle> targets;
            for (int i = 0; i < room->seatCount; ++i) {
                if (req->channel() == CHANNEL_TEAM && room->seats[i].team != senderTeam) {
                    continue;
                }
            ...
            lk.unlock();
            for (auto& h : targets) {
                m_push(h, MSG_CHAT_NOTIFY, payload);
            }
        }
    }
    resp->set_result(SUCCESS);
