...
//
// 替代旧的一维 heightMap(每列只有一个高度值, 无法表达
// "上方有平台、下方被掏空"的二维结构)。
//
...
//
// 坐标系: Y 向上, 原点在底部。
//   isSolid(x, y): 查坐标 (x, y) 处是否有地形。
//   removeCircle: 爆炸挖圆——只挖圆内格子, 圆外的平台保留。
//   columnHeight(x): 该列最高实体格的 y(等价旧 heightMap[x])。
//
// 用于:
//   - 弹道碰撞(computeHitPoint): isSolid(cx, cy) 替代 cy <= heightMap[ix]
//   - 爆炸挖坑(onShoot): removeCircle 替代整列降低
//   - 玩家站位(columnHeight): 该列地面高度
// ============================================================
class Terrain2D {
public:
