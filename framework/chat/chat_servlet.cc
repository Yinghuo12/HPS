...
int32_t SendMessage(sylar::http::WSSession::ptr session, ChatMessage::ptr msg) {
...
int32_t ChatWSServlet::onConnect(sylar::http::HttpRequest::ptr header, sylar::http::WSSession::ptr session) {
    SYLAR_LOG_INFO(g_logger) << "onConnect " << session;
    return 0;
}

int32_t ChatWSServlet::onClose(sylar::http::HttpRequest::ptr header, sylar::http::WSSession::ptr session) {
