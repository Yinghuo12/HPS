...
void test_pool() {
    sylar::http::HttpConnectionPool::ptr pool(new sylar::http::HttpConnectionPool("httpspot.dev", "", 80, 10, 1000 * 30, 5));
...
void run() {
    sylar::Address::ptr addr = sylar::Address::LookupAnyIPAddress("httpspot.dev:80");
    if(!addr) {
    ...
    req->setPath("/xml");
    req->setHeader("host", "httpspot.dev");
    ...
    auto r = sylar::http::HttpConnection::DoGet("http://httpspot.dev/xml", 300);
