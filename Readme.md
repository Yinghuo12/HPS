...
## 核心功能
### 客户端（OpenGL 3.3 + GLFW + ImGui）
- 回合制弹道射击：采用服务端权威物理引擎，客户端仅做插值播放与表现同步
- 房间系统：支持创建/加入/离开/准备/切换队伍，对局结束后自动返回房间，可立即开启下一局
- 聊天系统：实现 Team/All/Room/World/System/Broadcast/Private 七大频道通信
- 账号系统：提供注册/登录功能，基于 MySQL 持久化存储 + Redis 会话管理
- Protobuf 通信协议：包含 40+ 消息类型，使用 oneof payload 多态分发，支持 import 子 proto 模块化管理
- RPC 框架：基于 Protobuf + ZooKeeper 实现服务注册与发现
- 自动重连：断线后采用指数退避策略自动重连，重连后自动登录并恢复游戏状态

### 服务端（sylar C++ 协程框架）
- 基于 IOManager 协程调度器，单线程即可处理大量并发 WebSocket 连接
- Gate/Lobby/Match/Battle 四层分层架构，职责清晰解耦
- Actor per Room 模式：每个房间状态修改串行化，消除多线程竞争
- SessionManager 统一管理连接生命周期，封装踢人、心跳、断线清理
- 权威物理引擎：服务端计算完整弹道轨迹，客户端仅做插值渲染，杜绝物理外挂
- MySQL 连接池 + Redis 缓存：账号/对局/聊天持久化，会话 token 与在线状态管理
...
5. 特别鸣谢：[Claude Code] & [GLM Coding Plan] & [Gemini]
...
[C++ 并发编程(9) 两种并发设计模式Actor和CSP](https://www.bilibili.com/video/BV158411y7Bn/)

[【干货篇】云风-skynet手撕万人同时在线游戏](https://www.bilibili.com/video/BV1Lq4y1J7dH)
...
## version 13: 守护进程 (Daemon) 与进程级容灾保障
...
## Veision 14: 全局环境与命令行参数管理 (Env)

### 版本差异与核心演进亮点

在之前的版本中，如果我们要启动 HTTP 服务器，只能在 `main` 函数里把端口号或者配置文件路径写死。虽然 Version 13 实现了 Daemon 守护进程，但也只是粗暴地根据 `argc != 1` 来判断是否后台运行。

**Version 14 的核心升级如下：**
1. **全局统一管理 (`EnvMgr`)**：提供了一个全局单例，接管了程序启动时的 `argc` 和 `argv`，并在整个程序的生命周期内提供随处可用的查询接口。
2. **智能路径感知 (`m_exe` & `m_cwd`)**：不再依赖容易被欺骗的 `argv[0]`，而是下沉到 Linux 内核级别，利用 `/proc/pid/exe` 软链接精准定位当前程序的物理绝对路径和工作目录。
3. **命令行参数状态机解析**：支持标准的 `-key value` 以及无值的 `-key` (如 `-d` 开启后台运行) 解析，将其转化为 $O(1)$ 查询的 Hash Map。
4. **系统环境变量联动 (`setenv/getenv`)**：封装了对 Linux 系统级环境变量的读写，方便程序在 Docker 容器或 Kubernetes 环境中获取外部注入的环境变量配置。
5. **动态帮助文档生成 (`printHelp`)**：支持随时向系统中注册或注销命令参数的说明文档，并在输入非法参数时格式化对齐打印帮助列表。

---

### 模块全量核心类图 (UML)

以下是严格包含所有成员变量、宏定义、多重数据容器及所有函数签名的详尽 UML 类图：
...
classDiagram
    ...
    class Env {
        -m_mutex: RWMutexType
        -m_args: std::map~std::string, std::string~
        -m_helps: std::vector~std::pair<std::string, std::string>~
        -m_program: std::string
        -m_exe: std::string
        -m_cwd: std::string

        +init(argc: int, argv: char**): bool
        
        +add(key: const std::string&, val: const std::string&): void
        +has(key: const std::string&): bool
        +del(key: const std::string&): void
        +get(key: const std::string&, default_value: const std::string&): std::string

        +addHelp(key: const std::string&, desc: const std::string&): void
        +removeHelp(key: const std::string&): void
        +printHelp(): void

        +getExe(): const std::string& const
        +getCwd(): const std::string& const

        +setEnv(key: const std::string&, val: const std::string&): bool
        +getEnv(key: const std::string&, default_value: const std::string&): std::string
    }
    ...
    EnvMgr ..|> Singleton~Env~ : 实例化全局单例
    Env "1" *-- "n" m_args : 维护解析后的参数字典
    Env "1" *-- "n" m_helps : 维护按注册顺序排列的帮助列表
```
...
#### 1. 绝对路径侦测 (`init` 中的 `/proc` 技术)
* **痛点**：在 C/C++ 程序中，`argv[0]` 通常包含了程序启动时的命令。但如果用户是通过相对路径 `./bin/server` 启动，或者将程序加到了全局 `PATH` 后直接输入 `server` 启动，`argv[0]` 就无法给出程序到底在磁盘的哪个物理位置。如果程序内部需要加载同目录下的配置文件，就会导致严重的 `File Not Found`。
* **Sylar 的 Linux 内核级解法**：
  ```cpp
  char link[1024] = {0};
  char path[1024] = {0};
  ...
  ```
  在 Linux 系统中，`/proc/` 目录下包含了当前运行系统的所有实时内核数据。`/proc/[pid]/exe` 是一个特殊的**符号链接（Symbolic Link）**，它由内核强制维护，永远指向启动该进程的**真实物理二进制文件**的绝对路径！
  * 利用 `readlink` 解析该符号链接，直接拿到类似 `/home/user/sylar/bin/my_server` 的绝对路径赋给 `m_exe`。
  * 利用 `find_last_of("/")` 截断最后的文件名，提取出 `/home/user/sylar/bin/` 赋给 `m_cwd`。自此，程序拥有了绝对的工作目录感知能力。

#### 2. 命令行参数解析状态机 (`init` 核心循环)
Sylar 的命令行解析器需要处理形如：`./server -conf /etc/sylar.yml -d -w 4` 的复杂输入。

* **实现细节**：
  在 `init(int argc, char** argv)` 的 `for` 循环中，维护了一个状态指针 `const char* now_key = nullptr;`。
  1. **遇到前缀为 `-` 的参数**（如 `-conf`）：
     * 检查 `now_key` 是否为空。如果不为空，说明上一个 `-key` 后面跟着的不是值，而是另一个 `-key`（比如连续输入 `-d -w`）。此时主动把上一个 key 存入 Map，其 Value 设为空字符串 `""`。
     * 将当前的指针偏移 1 位（跳过 `-` 号），赋给 `now_key`。
  2. **遇到没有 `-` 的参数**（如 `/etc/sylar.yml`）：
     * 检查 `now_key` 是否为空。如果不为空，说明这个参数正是前面那个 `key` 的 `value`。调用 `add(now_key, argv[i])`，并将 `now_key` 置空，状态机复位。
     * 如果 `now_key` 为空，说明用户输入了游离的字符串（非法格式），直接打错误日志并返回 `false`。
  3. **收尾检查**：循环结束后，如果 `now_key` 还不为空，说明最后一个参数是形如 `-d` 的孤立标记，补充存入 Value 为 `""`。
* 整个过程时间复杂度为 $O(N)$，且完美兼容有值参数和无值布尔标记。

#### 3. 线程安全的多容器管理 (`m_args` 与 `m_helps`)
* 由于环境变量可能在程序运行期间被动态查询，而 `addHelp` 可能在不同模块的初始化阶段被跨线程调用，整个 `Env` 类被 `RWMutex` 读写锁全面保护。
* **数据结构选择的智慧**：
  * **参数存储 `m_args`** 选用 `std::map`（底层红黑树）：保障了 $O(\log N)$ 的高效跨线程并发查询性能。
  * **帮助文档 `m_helps`** 选用 `std::vector<std::pair>`：为什么不用 `map`？因为业务线在向核心注册参数说明时（例如 `addHelp("s", "start")`，`addHelp("d", "daemon")`），通常希望最终打印出来的帮助文档**保留代码里的注册先后顺序**。如果用 `map`，由于内部按字母重排，打印出来会显得杂乱无章。
  * 在 `removeHelp` 时，遍历 `vector` 利用迭代器安全地进行原地删除操作 (`it = m_helps.erase(it)`)。

#### 4. 打印排版艺术 (`std::setw`)
* 在 `printHelp` 中使用了 `std::setw(5)`：
  ```cpp
  std::cout << std::setw(5) << "-" << i.first << " : " << i.second << std::endl;
  ```
  这保证了在控制台输出时，即使参数名长度不一（比如短参数 `-d` 和长参数 `-conf`），它们后面的描述文本也会像表格一样对齐，提供了极致的开发者体验 (DX)。

#### 5. Linux 系统环境变量代理 (`getEnv` / `setEnv`)
* **痛点**：原生 C 库函数 `getenv` 如果获取不到环境变量，会返回一个危险的 `NULL` 裸指针，如果业务层直接拿去给 `std::string` 赋值会导致段错误崩溃。
* **Sylar 封装**：在 `getEnv` 内部拦截了 `nullptr`，并优雅地替换为开发者传入的 `default_value`，实现了绝对的空指针安全。在 `setEnv` 中调用系统 API `setenv(key, val, 1)`（第三个参数 `1` 代表强制覆盖现有变量）。
...
#### `test_env.cc`
这个测试用例极为巧妙，它不仅测试了本模块封装的 `EnvMgr`，还顺带演示了 C++ 在 `main` 函数之前执行代码的黑魔法。

1. **`main` 函数之前的执行 (`struct A`)**：
   * 定义了一个全局变量 `A a;`。在 C++ 规范中，全局对象的构造函数必定在 `main` 执行之前被调用。
   * 在 `A` 的构造函数中，它手动打开了 `/proc/[pid]/cmdline` 文件（这个文件里存放着启动该进程的最原始、未加任何修改的命令行参数二进制流）。将其原样读取并逐字节打印其 ASCII 码。这为对比我们自己解析的参数提供了最底层的内核级参照物。
2. **常规解析测试**：
   * 在 `main` 函数启动后，立刻向单例中注册了 3 个可用的系统指令：`-s`, `-d`, `-p`。
   * 调用 `init(argc, argv)` 进行解析。如果用户输入了乱七八糟的参数导致解析失败，立刻调用 `printHelp()` 打印对齐的帮助文档然后优雅退出程序。
3. **物理路径与环境变量测试**：
   * 打印出了利用 `/proc` 技术成功提取出的物理真实路径 `getExe()` 和纯净目录 `getCwd()`。
