...
void ServletDispatch::addGlobServlet(const std::string& uri, Servlet::ptr slt) {
    RWMutexType::WriteLock lock(m_mutex);
    // 避免同一个 uri 路径被重复添加，导致匹配错乱、优先级混乱
    // 1. 先删除旧的
...
int32_t NotFoundServlet::handle(sylar::http::HttpRequest::ptr request, sylar::http::HttpResponse::ptr response, sylar::http::HttpSession::ptr session) {
    static const std::string& RSP_BODY = "<html><head><title>404 Not Found"
        ...
        "<hr><center>yinghuo/1.0.0</center></body></html>";

    response->setStatus(sylar::http::HttpStatus::NOT_FOUND);  // 404
    response->setHeader("Server", "yinghuo/1.0.0");
