...
static sylar::ConfigVar<uint64_t>::ptr g_http_request_buffer_size = 
    sylar::Config::Lookup("http.request.buffer_size", (uint64_t)(4 * 1024), "http request buffer size");                   // 缓冲区大小
static sylar::ConfigVar<uint64_t>::ptr g_http_request_max_body_size = 
    sylar::Config::Lookup("http.request.max_body_size", (uint64_t)(64 * 1024 * 1024), "http request max body size");     // 最大请求体大小
static sylar::ConfigVar<uint64_t>::ptr g_http_response_buffer_size = 
    sylar::Config::Lookup("http.response.buffer_size", (uint64_t)(4 * 1024), "http response buffer size");                // 缓冲区大小
static sylar::ConfigVar<uint64_t>::ptr g_http_response_max_body_size = 
    sylar::Config::Lookup("http.response.max_body_size", (uint64_t)(64 * 1024 * 1024), "http response max body size");  // 最大请求体大小

static uint64_t s_http_request_buffer_size = 0;      // 缓冲区大小
static uint64_t s_http_request_max_body_size = 0;    // 最大请求体大小
static uint64_t s_http_response_buffer_size = 0;     // 缓冲区大小
static uint64_t s_http_response_max_body_size = 0;   // 最大请求体大小
...
// HTTP请求解析回调函数  传入的是数据的起始位置，如果是string, 还要复制一份，因此这里用了at和length参数，避免了string的复制 
