...
#include "sylar/scheduler/fd_manager.h"
#include "sylar/core/log.h"
#include "sylar/core/macro.h"
#include "sylar/scheduler/hook.h"
...
Address::ptr Socket::getLocalAddress() {
    ...
    socklen_t addrlen = result->getAddrLen();
    // 调用系统函数 getsockname () 获取本地真实地址(ip + port), 写到 result->getAddr 指向的内存里
...
void Socket::newSock() {
    ...
    if(SYLAR_LIKELY(m_sock != -1)) {  // SYLAR_LIKELY: 条件为真的概率很高 
        initSock();
    } else {
