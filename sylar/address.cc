...
// 根据主机名、地址族、套接字类型和协议查找并返回第一个匹配的地址
...
IPAddress::ptr Address::LookupAnyIPAddress(const std::string& host, int family, int type, int protocol) {
    /*
        根据给定的主机名（host）和地址族（family）、套接字类型（type）和协议（protocol）来查找并返回一个IP地址对象。
        如果找到多个IP地址，它只返回第一个有效的IP地址对象；如果没有找到，则返回nullptr。
    */
    std::vector<Address::ptr> result;
    if(Lookup(result, host, family, type, protocol)) {
        for(auto& i : result) {
...
// 工厂方法，自动识别地址类型
/*  传入底层 sockaddr 结构体, 自动判断是 IPv4 / IPv6 / Unix, 返回对应子类对象 */
...
std::ostream& IPv6Address::insert(std::ostream& os) const {
    ...
    bool used_zeros = false;   // 用于标记是否已经使用过::压缩（IPv6将连续的零段压缩用::压缩，并规定只能有一个::)
    for(size_t i = 0; i < 8; ++i) {
        // 如果当前段是 0，并且还没有使用过::压缩，则跳过（准备压缩）
        if(addr[i] == 0 && !used_zeros) {
            continue;
        }
        // 如果前一段是 0，且还没压缩过 则在这里插入::进行零段压缩
        if(i && addr[i - 1] == 0 && !used_zeros) {
            os << ":";  // 输出::中的第一个:
            used_zeros = true;
        }
        // 除了第一个段，其他段前面都要加 : 分隔
        // 如果当前段是0，在前面continue了，这一段不会执行
        if(i) {
            os << ":";   // 输出第二个:
        }
        // 输出当前 16 位段（十六进制格式）
        os << std::hex << (int)byteswapOnLittleEndian(addr[i]) << std::dec;  // 以十六进制格式输出,输出完切回十进制，避免影响后面打印

    }

    // 处理末尾全是 0 的特殊情况，例如 ::1 最后要补上 ::
...
// 获取 sockaddr_un.sun_path 的最大长度
static const size_t MAX_PATH_LEN = sizeof(((sockaddr_un*)0)->sun_path) - 1;  // -1：预留一个位置给字符串结束符 \0，防止越界
