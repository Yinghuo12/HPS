...
TcpServer::TcpServer(sylar::IOManager* woker, sylar::IOManager* accept_woker)
    :m_worker(woker)
    ,m_acceptWorker(accept_woker)
    ,m_recvTimeout(g_tcp_server_read_timeout->getValue())
    ,m_name("yinghuo/1.0.0")
    ,m_isStop(true) {
}
...
void TcpServer::stop() {
    ...
    // 捕获this不足以保护tcpServer对象不被销毁，this可能成为一个悬空指针，捕获this和self能更好保护对象在函数执行期间不被销毁
