...
// ============================================================
// RpcChannelPool — RPC 客户端连接池 + 服务发现缓存
//
// 解决两个性能问题(原 RpcChannel 每次 CallMethod 都做):
//   1. 每次都新建 etcd gRPC 连接做服务发现 —— 用带 TTL 的发现缓存消除。
//   2. 每次都新建 TCP socket + connect + close —— 用按 ip:port 池化的 socket 复用消除。
//
// 配合 sylar/rpc/rpc_provider.cc 的 keep-alive 改动(server 不再一连接一请求)使用。
//
// 线程/协程安全:
//   - 借出独占语义: 一个 socket 同时只被一个协程持有(acquire 返回后到 release 前),
//     sylar hook 的 send/recv 即便 yield 也不会出现同 socket 并发写。
//   - 与 orm/ConnectionPool 同模型(mutex + condition_variable)。
//
// 锁外执行耗时操作: connect 在锁外做(可能 yield, 避免长时间持锁阻塞其他协程)。
// ============================================================
...
public:
    ...
    explicit RpcChannelPool(const std::string& etcdEndpoint = "http://127.0.0.1:2379",
                            size_t maxSizePerHost = 8);
    ~RpcChannelPool();

    // 借一个到 ip:port 的 socket。无空闲且未达上限则新建并 connect;
    // 已达上限则阻塞等待(协程 yield)直到有空闲或容量。
    // 返回 nullptr 表示 connect 失败。
    ...
    class Guard {
    public:
        Guard(RpcChannelPool* pool, const std::string& ip, uint16_t port, sylar::Socket::ptr sock)
            : m_pool(pool), m_ip(ip), m_port(port), m_sock(sock) {}
        ~Guard() { if(m_pool && m_sock) m_pool->release(m_ip, m_port, m_sock); }
